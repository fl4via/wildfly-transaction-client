...
public final class SimpleXid implements Xid, Comparable<SimpleXid> {
    ...
    public static SimpleXid of(String hexString) {
        int globalIdIndex = hexString.indexOf(':') + 1;
        assert globalIdIndex > 2;
        int branchIdIndex = hexString.indexOf(':', globalIdIndex) + 1;
        assert branchIdIndex > globalIdIndex; // TODO replace by if throw exception
        int formatId = Integer.parseInt(hexString.substring(0, globalIdIndex -1), 16);
        byte[] globalId = bytesFromHexString(hexString.substring(globalIdIndex, branchIdIndex -1));
        byte[] branchId = bytesFromHexString(hexString.substring(branchIdIndex));
        return new SimpleXid(formatId, globalId, branchId, false);
    }

    private static byte[] bytesFromHexString(String hexString) {
